...
因此，GLM-5 最值得研究的不是某个孤立 benchmark 分数，而是它怎样把“模型能生成下一段文本”改造成“系统能完成一项不断改变状态的工作”。[GLM-5 引用图谱](../glm-5-reference-map.md)逐项梳理报告正文实际使用的 63 项来源；本页则沿报告结构重建模型、数据、优化、RL、系统与评测之间的因果链。
